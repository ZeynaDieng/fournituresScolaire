    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600">
          Gérez vos commandes, mettez à jour les statuts et synchronisez avec
          Google Sheets
        </p>
      </div>
        ...
        <div class="px-6 py-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex items-center p-4 bg-green-50 rounded-lg">
              ...
              <div class="ml-4">
                <p class="text-sm font-medium text-green-900">Stockage Local</p>
                <p class="text-sm text-green-700">✅ Opérationnel</p>
              </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-blue-50 rounded-lg">
              ...
              <div class="ml-4">
                <p class="text-sm font-medium text-blue-900">Fichier Excel</p>
                <p class="text-sm text-blue-700">✅ Synchronisé</p>
              </div>
            </div>
            ...
            <div
              ...
              :class="
                systemStatus.googleSheets.configured
                  ? 'bg-green-50'
                  : 'bg-orange-50'
              "
            >
              <div class="flex-shrink-0">
                ...
                >
                  <svg
                    v-if="systemStatus.googleSheets.configured"
                    class="w-5 h-5 text-white"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
        ...
        <div class="px-6 py-4">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <NuxtLink
              ...
              class="flex items-center justify-center px-4 py-3 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors"
            >
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button
              @click="downloadExcel"
              :disabled="loading"
              class="flex items-center justify-center px-4 py-3 border border-green-300 rounded-md text-green-700 hover:bg-green-50 disabled:opacity-50 transition-colors"
            >
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fill-rule="evenodd"
            ...
            <button
              ...
              "
              class="flex items-center justify-center px-4 py-3 border rounded-md disabled:opacity-50 transition-colors"
            >
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fill-rule="evenodd"
...
const checkSystemStatus = async () => {
  try {
    // Vérifier Google Sheets (simple check avec les variables d'env)
    const hasSheetId = !!process.env.GOOGLE_SHEET_ID;
    const hasApiKey = !!process.env.GOOGLE_SHEETS_API_KEY;

    systemStatus.value.googleSheets.configured = hasSheetId && hasApiKey;

    console.log("✅ État système vérifié");
  } catch (error) {
...
const loadStats = async () => {
  try {
    // Ici vous pourriez charger les vraies statistiques depuis l'API
...
const updateStatus = async () => {
  ...
  try {
    ...
    showMessage(
      "✅ Statut mis à jour avec succès! Fichier Excel synchronisé.",
      "success"
    );
...
const downloadExcel = async () => {
  try {
    ...
    // Ouvrir le fichier Excel dans un nouvel onglet
    window.open("/api/admin/orders/excel-master", "_blank");

    setTimeout(() => {
      showMessage("✅ Fichier Excel téléchargé!", "success");
    }, 1000);
  } catch (error) {
...
const syncGoogleSheets = async () => {
  if (!systemStatus.value.googleSheets.configured) {
    showMessage(
      "⚠️ Google Sheets n'est pas configuré. Vérifiez votre fichier .env",
      "warning"
    );
    return;
  }
  ...
  try {
    ...
    if (error.status === 401) {
      showMessage(
        "❌ Erreur d'authentification Google Sheets. Vérifiez votre API Key.",
        "error"
      );
    } else {
...
// Titre de la page
useHead({
  title: "Tableau de Bord Admin - Fournitures Scolaires",
});
</script>
