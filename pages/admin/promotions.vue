<template>
  <div class="container mx-auto py-8">
    ...
    <div class="mb-4 flex justify-between items-center">
      <button @click="showAdd = !showAdd" class="btn btn-primary">Ajouter une promotion</button>
    </div>
    <div v-if="showAdd" class="mb-6 p-4 bg-gray-50 rounded-lg">
      ...
      <form @submit.prevent="addPromotion">
        <input v-model="newPromotion.title" placeholder="Titre" class="form-input mb-2" required />
        <input v-model.number="newPromotion.discount" placeholder="Réduction (%)" type="number" class="form-input mb-2" required />
        <input v-model="newPromotion.type" placeholder="Type (percentage, fixed, bogo)" class="form-input mb-2" required />
        <input v-model="newPromotion.endDate" placeholder="Date de fin (YYYY-MM-DD)" class="form-input mb-2" required />
        <textarea v-model="newPromotion.description" placeholder="Description" class="form-input mb-2" />
