      ...
      <div v-if="cartItems.length === 0" class="text-center py-12">
        <div class="bg-white rounded-lg shadow-sm border p-8 max-w-md mx-auto">
          <svg class="mx-auto h-16 w-16 text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.5 6M7 13l-1.5-6m0 0h3m0 0V5a2 2 0 012-2v0a2 2 0 012 2v2m-6 0h6" />
          </svg>
          <h2 class="text-xl font-semibold text-gray-900 mb-2">Votre panier est vide</h2>
          <p class="text-gray-600 mb-4">Ajoutez des articles à votre panier pour continuer</p>
          <NuxtLink to="/" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700">
            ...
            <div class="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div class="space-y-2 pt-4 border-t border-gray-200">
                <div class="flex justify-between text-sm">
                  ...
                  <span class="text-gray-900">{{ formatAmount(subtotal) }}</span>
                </div>
                
                <div v-if="shippingCost > 0" class="flex justify-between text-sm">
                  <span class="text-gray-600">Livraison</span>
                  <span class="text-gray-900">{{ formatAmount(shippingCost) }}</span>
                </div>
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Finalisez votre commande de fournitures scolaires en toute sécurité avec PayTech' }
  ]
});
...
const totalAmount = computed(() => subtotal.value + shippingCost.value - discount.value);
...
const handlePaymentSuccess = (result: any) => {
  ...
  navigateTo(`/payment/success?ref=${result.ref_command}&amount=${result.amount}&method=${result.payment_method || ''}`);
};
