// utils/airtable-orders.        'Created Date': new Date().toISOString().split('T')[0], // Format YYYY-MM-DDs

async function addOrderToAirtable(orderData) {
  try {
...
async function updateOrderStatusInAirtable(orderRef, status) {
  try {
    const baseId = process.env.AIRTABLE_BASE_ID;
    const apiKey = process.env.AIRTABLE_API_KEY;
    const ordersTableId = process.env.AIRTABLE_ORDERS_TABLE;
