...
import { sendWhatsAppNotifications } from "~/utils/whatsapp-simple";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log("🧪 Test WhatsApp simple avec les données:", testInvoiceData);
    ...
    return {
      ...
      message: "Test WhatsApp simple terminé. Vérifiez les logs du serveur.",
    };
  } catch (error) {
    console.error("❌ Erreur lors du test WhatsApp simple:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "Erreur inconnue",
      message: "Échec du test WhatsApp simple.",
    };
  }
});
