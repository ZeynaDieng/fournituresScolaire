...
// Données locales des packs (sans dépendance Airtable)
const localPacks = [
  {
    id: "pack-cp",
...
export default defineEventHandler(async (event) => {
  try {
    console.log("📦 Récupération des packs depuis les données locales");

    // Transformer les données au format attendu
    const transformedPacks = localPacks.map((pack) => ({
      id: pack.id,
      name: pack.name,
      level: pack.level,
      price: pack.price,
    ...
    console.log(
      `✅ ${transformedPacks.length} packs chargés depuis les données locales`
    );
    ...
    console.error("❌ Erreur lors du chargement des packs locaux:", error);
