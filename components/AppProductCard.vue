<!-- ProductCard.vue -->
<template>
  <article class="group relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-[1.02] border border-slate-100 overflow-hidden">
    <!-- Background Animation -->
    <div class="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-emerald-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
    
    <!-- Promotion Badge -->
    <div v-if="product.isPromotion" class="absolute top-4 right-4 z-30">
      <div class="relative">
        <div class="bg-gradient-to-r from-red-500 to-rose-600 text-white text-sm font-bold px-3 py-2 rounded-2xl shadow-xl transform rotate-3 group-hover:rotate-0 transition-transform duration-300">
        ...
        <div class="absolute inset-0 bg-gradient-to-r from-red-500 to-rose-600 rounded-2xl blur-lg opacity-40 -z-10 animate-pulse"></div>
      </div>
    </div>
    ...
    <div class="absolute top-4 left-4 z-30">
      ...
      >
        <svg class="w-5 h-5 transition-transform group-hover/heart:scale-110" :fill="isWishlisted ? 'currentColor' : 'none'" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Image Container -->
    <div class="relative h-56 overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 rounded-t-3xl">
      <img 
        :src="product.image" 
        :alt="product.name"
    ...
    <div class="p-6 space-y-4 relative z-10">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
          <div class="flex items-center gap-0.5">
            <svg v-for="i in 5" :key="i" class="w-3 h-3" :class="i <= 4 ? 'text-yellow-400' : 'text-slate-200'" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div>
        <NuxtLink :to="`/products/${product.id}`" class="block group/title">
          <h3 class="text-xl font-bold text-slate-900 group-hover/title:text-emerald-700 transition-colors duration-300 line-clamp-2 leading-tight mb-2">
            {{ product.name }}
          </h3>
        </NuxtLink>
      ...
      <!-- Pricing -->
      <div class="bg-gradient-to-r from-slate-50 to-emerald-50/50 rounded-2xl p-4">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center bg-slate-100 rounded-xl overflow-hidden">
            ...
            <span class="px-4 py-2.5 font-semibold text-slate-900 min-w-[3rem] text-center bg-white">
              {{ quantity }}
            </span>
      ...
      <div v-if="bulkOptions.length > 0" class="pt-4 border-t border-slate-200">
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="text-sm font-medium text-slate-700">Tarifs dégressifs</span>
        </div>
        <div class="grid grid-cols-3 gap-2">
          ...
          >
            <div class="text-lg font-bold text-slate-900 group-hover/bulk:text-emerald-700">{{ option.quantity }}x</div>
            <div class="text-xs text-emerald-600 font-semibold">{{ formatPrice(option.unitPrice) }}</div>
    ...
    <!-- Countdown Timer -->
    <div v-if="product.isPromotion && product.promotionEndDate" class="px-6 pb-6">
      <AppCountdown 
    ...
    <!-- Shine Effect -->
    <div class="absolute inset-0 -top-2 -left-2 bg-gradient-to-r from-transparent via-white/30 to-transparent transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[400%] transition-transform duration-1000 ease-out"></div>
  </article>
</template>
...
const toggleWishlist = () => {
  ...
  if (process.client) {
    const wishlist = JSON.parse(localStorage.getItem('eduShopWishlist') || '[]')
