<template>
  <Transition name="slide-fade">
    <div v-if="cartStore.isOpen" class="fixed inset-0 z-50 flex justify-end" @click.self="cartStore.toggleCart">
      <div class="relative w-full max-w-md h-full bg-white shadow-2xl flex flex-col">
        ...
        <div class="flex items-center justify-between p-4 border-b">
          ...
          <button @click="cartStore.toggleCart" class="p-2 rounded-full hover:bg-gray-100">
        ...
        <!-- Cart Items -->
        <div v-if="cartStore.items.length > 0" class="flex-grow overflow-y-auto p-4 space-y-4">
          <div v-for="item in cartStore.items" :key="item.id" class="flex items-center space-x-4">
            <img :src="item.image" :alt="item.name" class="w-16 h-16 object-cover rounded-lg">
            <div class="flex-grow">
              <h3 class="font-semibold">{{ item.name }}</h3>
              <p class="text-sm text-gray-500">{{ item.price }} CFA x {{ item.quantity }}</p>
            </div>
            <div class="font-semibold">{{ item.price * item.quantity }} CFA</div>
        ...
        <div v-else class="flex-grow flex flex-col items-center justify-center text-center p-4">
            <svg class="w-16 h-16 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.5 5M4.5 20.5a.5.5 0 11-1 0 .5.5 0 011 0zm15 0a.5.5 0 11-1 0 .5.5 0 011 0z"></path></svg>
