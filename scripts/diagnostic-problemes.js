...
async function diagnosticComplet() {
  ...
  for (const test of apiTests) {
    ...
    log(`   ${status} ${test.endpoint}: ${result.status}`, result.success ? 'green' : 'red');
  }
  ...
  const contactData = {
    ...
    message: 'Message de test pour diagnostiquer les notifications email et WhatsApp'
  };
  ...
  const webhookResult = await testAPI('/api/paytech/webhook-new', 'POST', webhookData);
  if (webhookResult.success) {
    ...
    log(`   Erreur: ${webhookResult.error || 'Erreur ' + webhookResult.status}`, 'red');
  }
  ...
  const whatsappOrderData = {
    ...
    items: [
      { name: 'Pack CE2', quantity: 1, price: 30000 }
    ],
    totalAmount: 30000,
    phoneNumber: '+221771234567'
  };

  const whatsappResult = await testAPI('/api/orders/whatsapp', 'POST', whatsappOrderData);
