...
async function testEndpoint(url, method = 'GET', body = null) {
  return new Promise((resolve) => {
    const curlCmd = method === 'GET' 
      ...
      : `curl -s -w "%{http_code}" -o /dev/null -X ${method} -H "Content-Type: application/json" -d '${JSON.stringify(body)}' "${url}"`;
...
async function testWithResponse(url, method = 'GET', body = null) {
  return new Promise((resolve) => {
    const curlCmd = method === 'GET' 
      ? `curl -s "${url}"`
      : `curl -s -X ${method} -H "Content-Type: application/json" -d '${JSON.stringify(body)}' "${url}"`;
...
async function runTests() {
  ...
  const invoiceTest = await testEndpoint(`${BASE_URL}/api/orders/invoice/TEST_ORDER_123`);
  console.log(`   Statut: ${invoiceTest.status} ${invoiceTest.success ? '✅' : '❌'}`);
  
  console.log("\n2. 📋 Test nouveau endpoint mes commandes...");
  const ordersTest = await testWithResponse(`${BASE_URL}/api/orders/customer?email=test@example.com`);
  ...
  const contactData = {
    ...
    message: "Test pour voir si l'enregistrement Airtable fonctionne maintenant",
    phone: "221777123456"
  };
  
  const contactTest = await testWithResponse(`${BASE_URL}/api/contact/send`, 'POST', contactData);
  ...
  const webhookTest = await testWithResponse(`${BASE_URL}/api/paytech/webhook-new`, 'POST', webhookData);
