...
async function testNewEndpoints() {
  ...
  console.log(
    "\n1. Test GET /api/airtable/orders/[orderRef] (commande inexistante)"
  );
  try {
    const response = await fetch(
      `${BASE_URL}/api/airtable/orders/CMD_TEST_123`
    );
  ...
  console.log("\n2. Test GET /api/airtable/orders (liste des commandes)");
  try {
    const response = await fetch(`${BASE_URL}/api/airtable/orders?limit=5`);
    console.log(`Status: ${response.status}`);
    const data = await response.json();
  ...
  console.log("\n3. Test GET /api/airtable/orders/by-email (email inexistant)");
  try {
    const response = await fetch(
      `${BASE_URL}/api/airtable/orders/by-email?email=test@inexistant.com`
    );
    console.log(`Status: ${response.status}`);
    const data = await response.json();
    console.log("Réponse:", JSON.stringify(data, null, 2));
