...
async function forceDeploy() {
  ...
  try {
    ...
    exec('rm -rf .nuxt .output node_modules/.cache', 'Nettoyage du cache local');
    ...
    try {
      exec('vercel remove fournitures-scolaire --yes', 'Suppression projet Vercel existant');
    } catch (e) {
    ...
    const testUrl = 'https://fournitures-scolaire-8nq2b3vtb-nabous-projects.vercel.app';
    
    try {
      // Test simple avec curl
      const curlResult = execSync(`curl -s -w "HTTPSTATUS:%{http_code}" "${testUrl}/api/test"`, { encoding: 'utf8' });
      ...
      if (httpCode === '200') {
        ...
        // Test CORS headers
        const corsTest = execSync(`curl -s -I -H "Origin: https://test.com" "${testUrl}/api/test"`, { encoding: 'utf8' });
